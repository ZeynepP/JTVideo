...
class VideoClassificationLightningModule(pytorch_lightning.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        f1 = self.train_f1(F.softmax(y_hat, dim=-1), batch["label"])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        f1 = self.val_f1(F.softmax(y_hat, dim=-1), batch["label"])
        self.log("val_loss", loss)
        self.log(
            "val_acc", acc,  on_step=True, on_epoch=True, prog_bar=True, sync_dist=True
        )
        self.log("val_f1", f1, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True)
        return loss
...
def main():
    ...
    parser.add_argument("--clip_duration", default=10, type=float)
    parser.add_argument(
